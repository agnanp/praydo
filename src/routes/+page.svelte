<script lang="ts">
    ...
    let respraytime = $state();
    let latlong = $state();

    async function fetchGeocode() {
        const response = await geocode("Bantul");
        latlong = await response.json();
    }

    function getPrayTime() {
        const praytime = new PrayTime();
        praytime.location([-7.8982544, 110.3855534]);
        praytime.adjust({
            fajr: 20,
            dhuhr: "2 min",
            asr: "Standard",
        ...
        const now = new Date();
        respraytime = praytime.getTimes(now);
    }
    async function fetchLocation() {
        ...
        if (response.status === 200) {
            for (let i = 0; i < responseBody.data.length; i++) {
                const newItem = {
                    label: responseBody.data[i].lokasi,
                    value: responseBody.data[i].id,
                };
                const isDuplicate = location.some(
    ...
    function updateCountdown() {
        ...
        countdownInterval = setInterval(async () => {
            ...
            if (!nextPrayerFound) {
                ...
                if (
                    nextDaySchedule &&
                    nextDaySchedule.data &&
                    nextDaySchedule.data.jadwal
                ) {
                    const nextDayPrayers = [
                        {
                            name: "Imsak",
                            time: nextDaySchedule.data.jadwal.imsak,
                        },
                        {
                            name: "Subuh",
                            time: nextDaySchedule.data.jadwal.subuh,
                        },
                        {
                            name: "Terbit",
                            time: nextDaySchedule.data.jadwal.terbit,
                        },
                        {
                            name: "Dhuha",
                            time: nextDaySchedule.data.jadwal.dhuha,
                        },
                        {
                            name: "Dzuhur",
                            time: nextDaySchedule.data.jadwal.dzuhur,
                        },
                        {
                            name: "Ashar",
                            time: nextDaySchedule.data.jadwal.ashar,
                        },
                        {
                            name: "Maghrib",
                            time: nextDaySchedule.data.jadwal.maghrib,
                        },
                        {
                            name: "Isya",
                            time: nextDaySchedule.data.jadwal.isya,
                        },
                    ].filter(
                        (p) =>
                            selectedTimes.state.daily[
...
<main class="p-4 h-full">
    ...
    {#if schedule}
        ...
        <p class="text-center">
            {formatDateToIndonesian(schedule.data.jadwal.tanggal)}
        </p>
            ...
            >
                {#if selectedTimes.state.daily.imsak}
                    <div
                        ...
                        class:preset-tonal-primary={nextPrayerName === "Imsak"}
                        class:preset-filled-primary={nextPrayerName !== "Imsak"}
                    >
                        <p class="h3">Imsak</p>
                ...
                {#if selectedTimes.state.daily.subuh}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Subuh"}
                        class:preset-filled-primary={nextPrayerName !== "Subuh"}
                    >
                        <p class="h3">Subuh</p>
                ...
                {#if selectedTimes.state.daily.terbit}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Terbit"}
                        class:preset-filled-primary={nextPrayerName !==
                            "Terbit"}
                    >
                        <p class="h3">Terbit</p>
                ...
                {#if selectedTimes.state.daily.dhuha}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Dhuha"}
                        class:preset-filled-primary={nextPrayerName !== "Dhuha"}
                    >
                        <p class="h3">Dhuha</p>
                ...
                {#if selectedTimes.state.daily.dzuhur}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Dzuhur"}
                        class:preset-filled-primary={nextPrayerName !==
                            "Dzuhur"}
                    >
                        <p class="h3">Dzuhur</p>
                ...
                {#if selectedTimes.state.daily.ashar}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Ashar"}
                        class:preset-filled-primary={nextPrayerName !== "Ashar"}
                    >
                        <p class="h3">Ashar</p>
                ...
                {#if selectedTimes.state.daily.maghrib}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName ===
                            "Maghrib"}
                        class:preset-filled-primary={nextPrayerName !==
                            "Maghrib"}
                    >
                ...
                {#if selectedTimes.state.daily.isya}
                    <div
                        class="card p-4 text-center"
                        class:preset-tonal-primary={nextPrayerName === "Isya"}
                        class:preset-filled-primary={nextPrayerName !== "Isya"}
                    >
                        <p class="h3">Isya</p>
