...
vi.mock('$lib/store/selectedLocation', () => ({
    selectedLocation: { state: { latitude: -6.2088, longitude: 106.8456, label: 'Jakarta' } }
}));
vi.mock('$lib/store/calculationSettings', () => ({
    calculationSettings: { state: { method: 'NU', dhuhrMinutes: '0 min', asrMethod: 'Standard', highLatitudes: 'NightMiddle' } }
}));
vi.mock('$lib/store/selectedTimes', () => ({
    selectedTimes: { 
        state: { 
            format: '24h', 
            daily: { fajr: true, sunrise: true, dhuhr: true, asr: true, maghrib: true, isha: true } 
        } 
    }
}));
...
vi.mock('$lib/store/selectedAlert', () => ({
    selectedAlert: { state: { alert: { fajr: true, sunrise: true, dhuhr: true, asr: true, maghrib: true, isha: true } } }
}));
...
describe('PrayerManager', () => {
    ...
    it('should trigger notification at exact prayer time', async () => {
        ...
        expect(invokeMock).toHaveBeenCalledWith('send_native_notification', expect.objectContaining({
